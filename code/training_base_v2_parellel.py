...
def start_train(model, train_dataloader, val_dataloader,test_dataloader,  n_epochs, saving_dir, device_index, model_name):
    print_every = 1
    ...
    for epoch in tqdm(range(n_epochs)):        
        ...
        for batch_idx, (inputs, targets) in enumerate(train_dataloader):
            # inputs, targets = nn.DataParallel(inputs).to(device), nn.DataParallel(targets.squeeze_()).to(device)
